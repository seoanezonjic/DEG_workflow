...
```{r prior_proc, results='asis', echo=FALSE, warning=FALSE, message=FALSE}
	...
	create_clustering <- function(target, all_metrics, metrics_to_process, count_data, metric_var_2_legend_name){
		...
		samples_count_data <- samples_count_data[rowSums(edgeR::cpm(samples_count_data) > 0) == ncol(samples_count_data),] 
		...
		cat(paste(c(nrow(samples_count_data),"has been normalized."),sep = " "))
		...
		cat(paste(c(nrow(normalized_counts),"has been plotted."),sep = " "))
		...
		raw_labels <- names(samples_metrics_corrected)
		readable_labels <- metric_var_2_legend_name[match(raw_labels, metric_var_2_legend_name$metric_var), "legend_name" ]
...
```{r samples_clustering, results='asis', echo=FALSE, warning=FALSE, message=FALSE}
save.image("/home/bio267lab/proyectos/pmm2_belen/mRNAseq_2016/analysis/DEG_workflow/test.RData")
out_clustering <- NULL
...
#############
	...
	plot_in_div(create_clustering(target_obj, metric_table, vars_clustering, count_data, metric_var_2_legend_name)) #Nor recommended to set dinamic sizes
	"
