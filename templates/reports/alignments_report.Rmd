...
```{r prior_proc, echo=FALSE, warning=FALSE, message=FALSE}
	library(reshape2)
	library(ggplot2)

	metric_table <- data[['metric_table']]
	metric_var_2_legend_name <- data.frame(
		metric_var = c("initial_total_sequences", "final_total_sequences", "Uniquely_mapped_reads_.", "multi_map", "X._of_reads_mapped_to_multiple_loci", "X._of_reads_mapped_to_too_many_loci", "X._of_reads_unmapped._too_short", "X._of_reads_unmapped._other", "Uniquely_mapped_reads_number", "aligned_to_feature", "initial_weigthed_qual_per_sequence", "number_duplicatedreads.estimated.", "final_mean_qual_per_base", "final_min_qual_per_base_in_10th_decile", "final_min_qual_per_base_in_lower_quartile", "final_weigthed_qual_per_sequence"),
		legend_name = c("Total Reads Before Trimming", "Total Reads After Trimming", "Unique Map %", "Multi-mapping %", "Multi Map %", "Multi Map-too many %", "Unmapped: Too Short %", "Unmapped: Other %", "Unique Map", "Aligned to Genomic Feature", "Weighted Sequence Quality Before Trimming", "Estimated Duplicated Reads", "Average Base Quality", "Min Qual per Base (Tenth Decile)", "Min Qual per Base (Lower Quartile)", "Weighted Sequence Quality")
	)
...
```{r trimming, echo=FALSE, warning=FALSE, message=FALSE}
	cat("## **QC of reads before alignment**")
	cat("### **Total numbers of reads before and after trimming**")
...
```{r qual_scores, echo=FALSE, warning=FALSE, message=FALSE}
...
```{r aligned_v_qual, echo=FALSE, warning=FALSE, message=FALSE, eval = (experiment_type == "RNAseq")}
...
```{r mapping, echo=FALSE, warning=FALSE, message=FALSE, eval = (experiment_type == "RNAseq")}
...
```{r feature_overlap, echo=FALSE, warning=FALSE, message=FALSE, eval = (experiment_type == "RNAseq")}
