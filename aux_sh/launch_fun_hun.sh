...
do
	TARGET_N AME=`echo $TARGET_FILE | sed 's/_target.txt//'`
	...
	if [ "$functional_launch" == "4a" ]; then 
		/usr/bin/time -o $target_results_folder/process_data_functional_hunter -v functional_Hunter.R -c 3 $functional_hunter_options -i $target_results_folder -t E -o $target_results_folder/functional_enrichment &>$target_results_folder'/functional_Hunter.log'
