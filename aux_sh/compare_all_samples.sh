...
if [[ $experiment_type == "miRNAseq_detection" ]]; then
	...
	do
		...
		/usr/bin/time -o $target_results_folder/process_data_degenes_hunter -v degenes_Hunter.R $degenes_hunter_options  -q $query_genes -i $target_results_folder'/final_counts.txt' -o $target_results_folder &>$target_results_folder/'degenes_Hunter.log' &
	done
	wait
fi
